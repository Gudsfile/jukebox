...
def test_idle_tag_with_disc_returns_playing_and_play_command():
    state, cmd = transition_playback(Idle(), _event("tag-1"), DISC, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd == "play"


def test_idle_no_tag_stays_idle():
    state, cmd = transition_playback(Idle(), _event(None), NO_DISC_FOUND, CTX)
    assert state == Idle()
    assert cmd is None


def test_idle_unregistered_tag_stays_idle():
    state, cmd = transition_playback(Idle(), _event("tag-1"), NO_DISC_FOUND, CTX)
    assert state == Idle()
    assert cmd is None
...
def test_playing_same_tag_returns_fresh_playing_no_command():
    state, cmd = transition_playback(Playing(tag="tag-1"), _event("tag-1"), DISC, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd is None
...
def test_playing_same_tag_clears_any_pending_retry():
    ...
    state, cmd = transition_playback(playing, _event("tag-1"), DISC, CTX)
    assert state == Playing(tag="tag-1", retry=None)
    assert cmd is None


def test_playing_tag_removed_enters_waiting():
    state, cmd = transition_playback(Playing(tag="tag-1"), _event(None, 100.0), NO_DISC_FOUND, CTX)
    assert state == Waiting(tag="tag-1", removed_at=100.0)
    assert cmd is None


def test_playing_different_tag_with_disc_returns_play():
    state, cmd = transition_playback(Playing(tag="tag-1"), _event("tag-2"), DISC, CTX)
...
def test_playing_unregistered_tag_clears_cross_tag_retry():
    ...
    playing = Playing(tag="tag-1", retry=_retry("play", "tag-2"))
    state, cmd = transition_playback(playing, _event("tag-2"), NO_DISC_FOUND, CTX)
    assert state == Playing(tag="tag-1", retry=None)
    assert cmd is None


def test_playing_unregistered_tag_no_retry():
    state, cmd = transition_playback(Playing(tag="tag-1"), _event("tag-2"), NO_DISC_FOUND, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd is None
...
def test_waiting_same_tag_returns_playing_no_command():
    state, cmd = transition_playback(Waiting(tag="tag-1", removed_at=98.0), _event("tag-1", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd is None
...
def test_waiting_same_tag_clears_pause_retry():
    ...
    state, cmd = transition_playback(waiting, _event("tag-1", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-1", retry=None)
    assert cmd is None
...
def test_waiting_within_grace_period_stays():
    ...
    state, cmd = transition_playback(waiting, _event(None, 100.0), NO_DISC_FOUND, CTX)
...
def test_waiting_grace_period_expired_returns_paused_and_pause_command():
    ...
    state, cmd = transition_playback(Waiting(tag="tag-1", removed_at=96.0), _event(None, 100.0), NO_DISC_FOUND, CTX)
    assert state == Paused(tag="tag-1", paused_at=100.0)
    assert cmd == "pause"


def test_waiting_different_tag_with_disc_returns_play():
    state, cmd = transition_playback(Waiting(tag="tag-1", removed_at=98.0), _event("tag-2", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-2")
    assert cmd == "play"
...
def test_waiting_unregistered_tag_clears_retry():
    ...
    waiting = Waiting(tag="tag-1", removed_at=98.0, retry=_retry("pause"))
    state, cmd = transition_playback(waiting, _event("tag-2", 100.0), NO_DISC_FOUND, CTX)
...
def test_paused_same_tag_within_max_returns_playing_and_resume():
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=80.0), _event("tag-1", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd == "resume"
...
def test_paused_same_tag_max_exceeded_returns_playing_and_play():
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=40.0), _event("tag-1", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd == "play"


def test_paused_different_tag_with_disc_returns_play():
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=80.0), _event("tag-2", 100.0), DISC, CTX)
    assert state == Playing(tag="tag-2")
    assert cmd == "play"
...
def test_paused_unregistered_tag_clears_retry():
    ...
    state, cmd = transition_playback(paused, _event("tag-2", 100.0), NO_DISC_FOUND, CTX)
...
def test_paused_no_tag_within_max_stays():
    paused = Paused(tag="tag-1", paused_at=80.0)
    state, cmd = transition_playback(paused, _event(None, 100.0), NO_DISC_FOUND, CTX)
...
def test_paused_no_tag_max_exceeded_returns_idle_and_stop():
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=40.0), _event(None, 100.0), NO_DISC_FOUND, CTX)
    assert state == Idle()
    assert cmd == "stop"
...
def test_paused_same_tag_max_exceeded_no_disc_stays_paused():
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=40.0), _event("tag-1", 100.0), NO_DISC_FOUND, CTX)
...
def test_paused_same_unregistered_tag_still_resumes():
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=80.0), _event("tag-1", 100.0), NO_DISC_FOUND, CTX)
    assert state == Playing(tag="tag-1")
    assert cmd == "resume"
...
def test_paused_within_max_pause_boundary(paused_seconds_ago):
    ...
    state, cmd = transition_playback(Paused(tag="tag-1", paused_at=paused_at), _event(None, 100.0), NO_DISC_FOUND, CTX)
...
def test_waiting_within_grace_boundary(removal_seconds_ago):
    ...
    state, cmd = transition_playback(
        Waiting(tag="tag-1", removed_at=removed_at), _event(None, 100.0), NO_DISC_FOUND, CTX
    )
