...
```shell
jukebox-admin settings set jukebox.reader.type nfc
```
...
| --- | --- | --- |
| `jukebox.reader.type` | Persists the reader choice across restarts (avoids passing `--reader` each time) | `dryrun` |
| `jukebox.reader.nfc.read_timeout_seconds` | Timeout in seconds for each NFC poll attempt (must be > 0) | `0.1` |

Use `jukebox-admin settings set` to change the timeout:

```shell
jukebox-admin settings set jukebox.reader.nfc.read_timeout_seconds 0.2
```
